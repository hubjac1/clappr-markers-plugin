...
/***/ (function(module, exports, __webpack_require__) {
	...
	_this3.core.mediaControl.container.seek(marker.time);e.preventDefault();e.stopImmediatePropagation();});this._$markers.appendChild($marker);// tooltip
	var $tooltip=marker.$tooltip;if($tooltip){console.log('there is a tooltip');// there is a tooltip
	...
	return;}this._markers.forEach(function(marker){marker.source.render();_this5._updateTooltipPosition(marker);});}},{key:'_appendElToMediaControl',value:function _appendElToMediaControl(){this.core.mediaControl.$el.find(".bar-container").first().append(this.el);}},{key:'render',value:function render(){this._$markers=document.createElement('div');this._$markers.classList.add("markers-plugin-markers");this._$tooltips=document.createElement('div');this._$tooltips.classList.add("markers-plugin-tooltips");this.el.appendChild(this._$markers);this.el.appendChild(this._$tooltips);this._appendElToMediaControl();return this;}},{key:'destroy',value:function destroy(){// remove any listeners and call onDestroy()
...
/***/ (function(module, exports, __webpack_require__) {
	 ...
	 */var CropMarker=function(_BaseMarker){_inherits(CropMarker,_BaseMarker);/*
	   ...
	   */function CropMarker(time,duration){_classCallCheck(this,CropMarker);var _this=_possibleConstructorReturn(this,(CropMarker.__proto__||Object.getPrototypeOf(CropMarker)).call(this,time));_this._duration=duration||5;_this.core=undefined;return _this;}/*
	   * Returns the duration (in seconds) that this marker represents.
	   */_createClass(CropMarker,[{key:'getDuration',value:function getDuration(){var duration=this._updateDurationValueFromCss();return duration;}},{key:'_updateDurationValueFromCss',value:function _updateDurationValueFromCss(){var duration=this.videoDuration/100*parseFloat(this.getMarkerEl().style.width);if(isNaN(duration)){duration=this._duration;}else{this._duration=duration;}return duration;}/*
	   * Set the duration (in seconds) that this marker should represents.
	   */},{key:'setDuration',value:function setDuration(duration){var fitToChunk=arguments.length>1&&arguments[1]!==undefined?arguments[1]:false;// TODO manager 0
	if(this._duration!==duration){this._renderDuration(duration);this._duration=duration;}}},{key:'_renderDuration',value:function _renderDuration(duration){var percentage=Math.min(Math.max(this._time/this.videoDuration*100,0),100);var percentageWidth=Math.min(Math.max(duration/this.videoDuration*100,0),100-percentage);this.getMarkerEl().style.width=percentageWidth+"%";//console.log("Width:" + percentageWidth + "%")
	}},{key:'_renderTime',value:function _renderTime(time){var percentage=Math.min(Math.max(time/this.videoDuration*100,0),100);this.getMarkerEl().style.left=percentage+"%";//console.log("Left:" + percentage + "%")
	}},{key:'render',value:function render(){//console.log('render')
	this._renderDuration(this._duration);this._renderTime(this._time);}/*
		 * Should return the time (in seconds) that this marker represents.
		 */},{key:'getTime',value:function getTime(){var time=this._updateTimeFromCss();return time;}},{key:'_updateTimeFromCss',value:function _updateTimeFromCss(){var time=this.videoDuration/100*parseFloat(this.getMarkerEl().style.left);if(isNaN(time)){time=this._time;}else{this._time=time;}return time;}/*
		 * Set the time (in seconds) that this marker represents.
		 */},{key:'setTime',value:function setTime(time){if(this._time!==time){this._renderTime(time);this._time=time;}}},{key:'_buildMarkerEl',value:function _buildMarkerEl(){var _this2=this;if(typeof jQuery!=='undefined'){var $marker=(0,_clappr.$)('<div />').addClass('crop-marker');$marker.append((0,_clappr.$)('<div id="wgrip"/>').addClass('crop-marker-handle').addClass('ui-resizable-handle').addClass('ui-resizable-w').addClass('left'));$marker.append((0,_clappr.$)('<div id="egrip"/>').addClass('crop-marker-handle').addClass('ui-resizable-handle').addClass('ui-resizable-e').addClass('right'));jQuery($marker[0]).resizable({handles:{'w':'#wgrip','e':'#egrip'},resize:function resize(event,ui){ui.originalElement.css({'left':''+1.0*ui.position.left/ui.originalElement.parents('.bar-container').width()*100.0+'%','width':''+1.0*ui.size.width/ui.originalElement.parents('.bar-container').width()*100.0+'%'});_this2._updateDurationValueFromCss();_this2._updateTimeFromCss();},maxHeight:20,minHeight:20});return $marker[0];}}},{key:'getHlsFragments',value:function getHlsFragments(){var updateToFit=arguments.length>0&&arguments[0]!==undefined?arguments[0]:false;try{var playback=this.core.getCurrentPlayback();if(playback.name==='hls'){var videoHlsLevels=playback._hls.levels;}else{throw new Error('this video does not contains fragments');}var fragments=videoHlsLevels[0].details.fragments;var startTime=this.getTime();var endTime=this.getDuration()+startTime;var coversFragments=fragments.filter(function(frag){var midTs=frag.start+frag.duration/2;return startTime<midTs&&endTime>midTs;});if(updateToFit){startTime=coversFragments[0].start;this.setTime(startTime);var lastFrag=coversFragments[coversFragments.length-1];this.setDuration(lastFrag.endDTS-startTime);}return coversFragments.map(function(frag){return frag.relurl;});}catch(e){if(this._fitToChunk){console.error('error in getting HLS chunk');console.error(e.message);}else{//ignor error
	}return[];}}}]);return CropMarker;}(_baseMarker2.default);exports.default=CropMarker;module.exports=exports['default'];
